    ...
    class claim_model extends base_model {
        ...
        public function createClaimData($staff_id){
        
            $str = '1234567890';
        $shuffled = str_shuffle($str);
        $dbData =array();
        $dbData["token"] = $this->generateToken($staff_id);
    ...
    public function generateToken($staff_id){
        $eToken="claim_";
        $token = sha1($eToken."-".$staff_id."-".currentTimeMilisec()."-".rand(0,10000));
        return $token;
    }
    public function getEnableStaffByClaimData($claim_token,$otp){
        $db = $this->getDb();
